...
| **Utilities** | Allow-listed scripts only (`ALLOWED_UTILITIES`) | `disk-wizard` Status/Run are read-only; apply stays on the CLI |
...
<Callout type="warning" title="docker.sock blast radius">
