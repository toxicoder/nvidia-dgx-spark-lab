...
<Callout type="error" title="Never commit live secrets">
  No kubeconfig with credentials, no `ansible/inventory/hosts.ini`, no host SSH private keys (`secrets/identities/`), no `GROK_DEPLOYMENT_KEY`, no Hermes `~/.hermes` / `hermes/data/`, no `.env`. Policy: [SECURITY.md](https://github.com/toxicoder/nvidia-dgx-spark-lab/blob/main/SECURITY.md).
</Callout>
## Templates (committed)
...
| `secrets/identities/README.md` | `secrets/identities/lab-ansible` (gitignored key pairs) |
...
Host SSH keys for `lab-admin` / `lab-ansible` are a separate store (`secrets/identities/`, not the dashboard vault). See [Lab identities](identities.md).
