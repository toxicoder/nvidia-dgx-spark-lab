...
<Callout type="info" title="Contributor laptop vs Spark host">
...
flowchart TB
  subgraph host ["Spark host"]
    pf["kubectl port-forward to nemotron :8000"]
    subgraph hermes ["hermes container (host network)"]
      api[":8642 OpenAI-compatible gateway"]
      dash[":9119 Web dashboard"]
      mcp["MCP 127.0.0.1:32100-32106"]
    end
    k8s["K8s agent-tools + ai-inference"]
  end
  pf --> hermes
  hermes --> k8s
```
