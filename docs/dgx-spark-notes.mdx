...
<Tabs groupId="1-node-2-node-5-node" items={["1-node", "2-node", "3-node ring", "4/5-node switch"]} persist>
  ...
  <Tab value="2-node">
    **2-node pair (dual ~400G aggregate):** existing Jobs and `group_vars` `highspeed_*` /
  ...
  <Tab value="4/5-node switch">
    **4 / 5-node CRS804 switch:** each node sees **one** RoCE link — one full port per node
    (400G) for 4 nodes, or three full ports plus one port split into `sfpplus4a` / `sfpplus4b`
    (two 200G lanes) for 5. Every link is its own /24 (192.168.110–114, MTU 9000) from the
