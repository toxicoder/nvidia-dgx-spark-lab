...
| `.github/workflows/publish-images.yml` | Digest-then-manifest publish to GHCR (`GITHUB_TOKEN` + `packages: write`). Gitea has no GHCR; keep building `lab-*:local` on the node until a registry is chosen. |
