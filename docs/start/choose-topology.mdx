...
| **2** | `pair` | dual QSFP direct between the two Sparks | `kimi-test`, then 2-node models |
...
- **4-node lab:** each Spark plugs into one of the four `sfpplus1`…`sfpplus4` ports at 400G.
...
| **2** | control-plane + worker | worker | dual QSFP ~400G aggregate |
