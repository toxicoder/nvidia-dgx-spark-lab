...
Multi-node nodes are labeled `nvidia-dgx-spark/highspeed=true` (Ansible labels role). Jobs that prefer the fabric use `operator: Exists` on that key — the value is not a speed rating.
