...
```mermaid
flowchart TD
  Policy["resource-policy.yaml / json"] --> Runtime["resources.sh preflight"]
  Runtime --> K8s["PriorityClass / Quota / LimitRange"]
  K8s --> Host["kubelet reserved"]
  Host --> SSH["SSH and control plane stay up"]
```
