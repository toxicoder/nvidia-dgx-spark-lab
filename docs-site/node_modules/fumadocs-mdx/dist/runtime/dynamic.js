...
import { t as buildMDX } from "../build-BPukCt-X.js";
...
async function dynamic(configExports, coreOptions) {
	...
	function convertLazyEntries(collection, entries) {
		...
		async function compile({ info, data }) {
			...
			return await executeMdx((await buildMDX(core, collection, {
				filePath: info.fullPath,
				source: content,
				frontmatter: data,
				isDevelopment: false,
				environment: "runtime"
			})).code, { baseUrl: pathToFileURL(info.fullPath) });
		}
