import { n as ident, r as slash, t as createCodegen } from "./codegen-BYYNMDsX.js";
...
function indexFile(options = {}) {
	...
	function generateTypeConfig(core) {
...
async function generateServerIndexFile(ctx) {
	...
	codegen.lines.push(`import { server } from 'fumadocs-mdx/runtime/server';`, `import type * as Config from '${codegen.formatImportPath(core.configPath)}';`, "", `const create = server<typeof Config, ${tc}>();`);
...
async function generateDynamicIndexFile(ctx) {
	...
	codegen.lines.push(`import { dynamic } from 'fumadocs-mdx/runtime/dynamic';`, ...hasDynamicCollection ? [`import path from 'node:path';`] : [], `import * as Config from '${codegen.formatImportPath(core.configPath)}';`, "", `const create = await dynamic<typeof Config, ${tc}>(Config, ${JSON.stringify(partialOptions)});`);
	codegen.lines.push(...objects.filter((obj) => obj !== void 0));
}
async function generateBrowserIndexFile(ctx) {
	const { core, codegen, tc } = ctx;
	codegen.lines.push(`import { browser } from 'fumadocs-mdx/runtime/browser';`, `import type * as Config from '${codegen.formatImportPath(core.configPath)}';`, "", `const create = browser<typeof Config, ${tc}>();`);
	async function generateCollectionObject(collection) {
		switch (collection.type) {
			case "docs":
				if (collection.docs.dynamic) return;
