...
import { MULTILINGUAL_LANGUAGE, SPLITTERS, SUPPORTED_LANGUAGES, LANGUAGES_WITH_SIGNIFICANT_DIACRITICS } from "./languages.js";
...
function tokenize(input, language, prop, withCache = true) {
	...
	const parts = this.language === MULTILINGUAL_LANGUAGE ? splitMultilingual(input) : input.toLowerCase().split(SPLITTERS[this.language]);
	const tokens = [];
...
export function createTokenizer(config = {}) {
	...
	const tokenizer = {
		...
		stopWords,
		stopWordsSet: stopWords ? new Set(LANGUAGES_WITH_SIGNIFICANT_DIACRITICS.has(config.language) ? stopWords : stopWords.map(replaceDiacritics)) : undefined,
