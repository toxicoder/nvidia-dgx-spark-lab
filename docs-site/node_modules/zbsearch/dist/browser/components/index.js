...
export function insertDocumentScoreParameters(index, prop, id, tokens, docsCount, internalId) {
...
export function insertTokenScoreParameters(index, prop, id, tokens, token, internalId) {
...
export function insertRadixTokens(index, prop, node, id, internalId, tokens, docsCount) {
...
export function removeTokenScoreParameters(_index, _prop, _token) {
	// Document frequency is derived from radix postings at search time.
}
export function create(zbsearch, sharedInternalDocumentStore, schema, index, prefix = "") {
	if (!index) {
		index = {
...
export function addSearchablePropertyToIndex(zbsearch, index, path, type) {
...
const PREFIX_EXPANSION_SCORE_DEMOTION = .5;
...
function searchThresholdZero(index, tokens, propertiesToSearch, exact, tolerance, boost, relevance, docsCount, whereFiltersIDs) {
	...
	for (const prop of propertiesToSearch) {
		...
		for (let i = 0; i < tokenLength; i++) {
			...
			propertyIntersection = propertyIntersection === undefined ? tokenIds : setIntersection(propertyIntersection, tokenIds);
		...
		for (let i = 0; i < tokenLength; i++) {
			...
			for (let j = 0; j < words.length; j++) {
				...
				if (filteredIds.length > 0) {
					calculateResultScores(index, prop, word, filteredIds, docsCount, relevance, resultsMap, boostPerProperty * (word === token ? 1 : prefixExpansionDemotion(searchResult[token]?.length, searchResult[word].length, docsCount)), whereFiltersIDs);
				}
			}
		}
	}
...
export function search(index, term, tokenizer, language, propertiesToSearch, exact, tolerance, boost, relevance, docsCount, whereFiltersIDs, threshold = 0, prefix = true) {
...
export function searchSuggestions(index, queryTokens, propertiesToSearch, boost, relevance, docsCount, whereFiltersIDs) {
	...
	for (const prop of propertiesToSearch) {
		...
		for (let i = 0; i < tokenCount; i++) {
			...
			for (let j = 0; j < words.length; j++) {
				...
				if (wholeWordWithTolerance && word !== token && (Math.abs(word.length - token.length) > tolerance || levenshtein(token, word) > tolerance)) {
					continue;
				}
				const wordBoost = boostPerProperty * (word === token ? 1 : prefixExpansionDemotion(tokenDocumentFrequency, ids.length, docsCount));
				...
				for (let k = 0; k < ids.length; k++) {
					...
					const score = BM25(tf, termOccurrences, docsCount, fieldLengths[internalId], avgFieldLength, relevance) * wordBoost;
...
export function load(sharedInternalDocumentStore, raw, indexesConfig) {
...
export async function loadAsync(sharedInternalDocumentStore, raw, indexesConfig) {
	const { indexes: rawIndexes, vectorIndexes: rawVectorIndexes, searchableProperties, searchablePropertiesWithTypes, frequencies, tokenOccurrences, avgFieldLength, fieldLengths } = raw;
	const indexes = {};
	const vectorIndexes = {};
