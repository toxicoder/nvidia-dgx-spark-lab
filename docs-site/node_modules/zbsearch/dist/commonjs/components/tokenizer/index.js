...
function normalizeToken(prop, token, withCache = true) {
    ...
    if (!languages_js_1.LANGUAGES_WITH_SIGNIFICANT_DIACRITICS.has(this.language)) {
...
function tokenize(input, language, prop, withCache = true) {
    ...
    const parts = this.language === languages_js_1.MULTILINGUAL_LANGUAGE
        ? splitMultilingual(input)
        : input.toLowerCase().split(languages_js_1.SPLITTERS[this.language]);
    const tokens = [];
...
function createTokenizer(config = {}) {
    ...
    else if (config.language !== languages_js_1.MULTILINGUAL_LANGUAGE && !languages_js_1.SUPPORTED_LANGUAGES.includes(config.language)) {
    ...
    const tokenizer = {
        ...
        stopWordsSet: stopWords
            ? new Set(languages_js_1.LANGUAGES_WITH_SIGNIFICANT_DIACRITICS.has(config.language) ? stopWords : stopWords.map(diacritics_js_1.replaceDiacritics))
            : undefined,
