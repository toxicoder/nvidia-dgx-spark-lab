...
exports.OBJECT_COMPONENTS = ['tokenizer', 'index', 'documentsStore', 'sorter', 'pinning'];
...
exports.SINGLE_OR_ARRAY_COMPONENTS = [ /* deprecated with v2.0.0-beta.5 */];
function runSingleHook(hooks, zbsearch, id, doc) {
    if (!hooks.length) {
        return;
    }
...
function runMultipleHook(hooks, zbsearch, docsOrIds) {
    if (!hooks.length) {
        return;
    }
    const needAsync = hooks.some(utils_js_1.isAsyncFunction);
