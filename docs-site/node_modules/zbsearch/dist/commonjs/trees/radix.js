...
const postings_js_1 = require("./postings.js");
const EMPTY_POSTINGS = [];
class RadixNode {
    // Node key
    k;
    // Node subword
    ...
    _findLevenshtein(term, tolerance, output, postings) {
        ...
        const stack = [{ node: this, row: initialRow }];
        while (stack.length > 0) {
            ...
            for (const child of node.c.values()) {
                ...
                for (let charIndex = 0; charIndex < label.length; charIndex++) {
                    ...
                    for (let i = 1; i <= termLength; i++) {
                        const value = term.charCodeAt(i - 1) === charCode
                            ? currentRow[i - 1]
    ...
    removeWordWithPostings(term, postings) {
        ...
        (0, postings_js_1.clearPostings)(postings, node.w);
        node.d = undefined;
        node.e = false;
...
class RadixTree extends RadixNode {
    postings = (0, postings_js_1.createPostingsMap)();
    constructor() {
        super('', '', false);
    }
    insert(word, docId) {
    ...
    getDocumentFrequency(term) {
        return (0, postings_js_1.getDocumentFrequency)(this.postings, term);
    }
    toJSON() {
        return {
            ...this.toNodeJSON(),
            postings: (0, postings_js_1.serializePostingsMap)(this.postings)
        };
    }
    static fromJSON(json) {
        ...
        if (json.postings) {
            tree.postings = (0, postings_js_1.deserializePostingsMap)(json.postings);
        }
        else {
            (0, postings_js_1.collectLegacyNodePostings)(json, tree.postings);
        }
