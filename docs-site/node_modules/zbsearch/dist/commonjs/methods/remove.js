...
function remove(zbsearch, id, language, skipHooks) {
    const asyncNeeded = (0, utils_js_1.isAsyncFunction)(zbsearch.index.beforeRemove) ||
        (0, utils_js_1.isAsyncFunction)(zbsearch.index.remove) ||
        (0, utils_js_1.isAsyncFunction)(zbsearch.index.afterRemove);
    if (asyncNeeded) {
...
async function removeAsync(zbsearch, id, language, skipHooks) {
    ...
    const internalId = (0, internal_document_id_store_js_1.getInternalDocumentId)(zbsearch.internalDocumentIDStore, id);
    const docId = (0, internal_document_id_store_js_1.getDocumentIdFromInternalId)(zbsearch.internalDocumentIDStore, internalId);
    const docsCount = zbsearch.documentsStore.count(docs);
    if (!skipHooks) {
        await (0, hooks_js_1.runSingleHook)(zbsearch.beforeRemove, zbsearch, docId);
    }
    ...
    if (!skipHooks) {
        await (0, hooks_js_1.runSingleHook)(zbsearch.afterRemove, zbsearch, docId);
    }
...
function removeSync(zbsearch, id, language, skipHooks) {
    ...
    const docsCount = zbsearch.documentsStore.count(docs);
    if (!skipHooks) {
        (0, hooks_js_1.runSingleHook)(zbsearch.beforeRemove, zbsearch, docId);
    }
    const indexableProperties = zbsearch.index.getSearchableProperties(index);
    const indexablePropertiesWithTypes = zbsearch.index.getSearchablePropertiesWithTypes(index);
    const values = zbsearch.getDocumentProperties(doc, indexableProperties);
    ...
    if (!skipHooks) {
        (0, hooks_js_1.runSingleHook)(zbsearch.afterRemove, zbsearch, docId);
    }
    zbsearch.documentsStore.remove(zbsearch.data.docs, id, internalId);
    return result;
}
function removeMultiple(zbsearch, ids, batchSize, language, skipHooks) {
    const asyncNeeded = (0, utils_js_1.isAsyncFunction)(zbsearch.index.beforeRemove) ||
        (0, utils_js_1.isAsyncFunction)(zbsearch.index.remove) ||
        (0, utils_js_1.isAsyncFunction)(zbsearch.index.afterRemove) ||
        (0, utils_js_1.isAsyncFunction)(zbsearch.beforeRemoveMultiple) ||
        (0, utils_js_1.isAsyncFunction)(zbsearch.afterRemoveMultiple);
    if (asyncNeeded) {
...
async function removeMultipleAsync(zbsearch, ids, batchSize, language, skipHooks) {
    ...
    const docIdsForHooks = skipHooks
        ? []
        : ids.map((id) => (0, internal_document_id_store_js_1.getDocumentIdFromInternalId)(zbsearch.internalDocumentIDStore, (0, internal_document_id_store_js_1.getInternalDocumentId)(zbsearch.internalDocumentIDStore, id)));
    if (!skipHooks) {
        await (0, hooks_js_1.runMultipleHook)(zbsearch.beforeRemoveMultiple, zbsearch, docIdsForHooks);
    }
    ...
    if (!skipHooks) {
        await (0, hooks_js_1.runMultipleHook)(zbsearch.afterRemoveMultiple, zbsearch, docIdsForHooks);
    }
    return result;
}
function removeMultipleSync(zbsearch, ids, batchSize, language, skipHooks) {
    ...
    const docIdsForHooks = skipHooks
        ...
        : ids.map((id) => (0, internal_document_id_store_js_1.getDocumentIdFromInternalId)(zbsearch.internalDocumentIDStore, (0, internal_document_id_store_js_1.getInternalDocumentId)(zbsearch.internalDocumentIDStore, id)));
    if (!skipHooks) {
        (0, hooks_js_1.runMultipleHook)(zbsearch.beforeRemoveMultiple, zbsearch, docIdsForHooks);
    }
    ...
    if (!skipHooks) {
        (0, hooks_js_1.runMultipleHook)(zbsearch.afterRemoveMultiple, zbsearch, docIdsForHooks);
    }
    return result;
}
