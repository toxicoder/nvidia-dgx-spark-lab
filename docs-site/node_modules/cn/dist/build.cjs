...
const walk = (dir, rel, out, optIn, ancestors, state) => {
	let real;
	try {
		real = (0, node_fs.realpathSync)(dir);
	} catch {
	...
	for (const e of entries) {
		...
		const childAbs = (0, node_path.join)(dir, e.name);
		let isDir = e.isDirectory();
		let isFile = e.isFile();
		if (e.isSymbolicLink()) try {
			const st = (0, node_fs.statSync)(childAbs);
...
const expandGlobs = (patterns, cwd, state = { skippedUnreadable: 0 }) => {
	...
	for (const raw of patterns) {
		...
		if (!/[*?{]/.test(pattern)) {
			const p = (0, node_path.resolve)(cwd, pattern);
			let st;
			try {
				st = (0, node_fs.statSync)(p);
			} catch {
				continue;
			}
			if (st.isFile()) files.add(p);
				...
				for (const f of sub) files.add((0, node_path.join)(p, f.split("/").join(node_path.sep)));
			}
			continue;
		}
		if (allFiles === null) {
		...
		for (const f of allFiles) if (re.test(f)) files.add((0, node_path.resolve)(cwd, f.split("/").join(node_path.sep)));
	}
	return [...files];
};
/**
...
const createContentMatcher = (patterns, cwd) => {
	...
	return (file) => {
		const rel = (0, node_path.relative)(cwd, file).split(node_path.sep).join("/");
...
const loadConfig = async (cwd, file) => {
	let mod;
	try {
		mod = await import((0, node_url.pathToFileURL)((0, node_path.resolve)(cwd, file)).href);
	} catch (err) {
...
const build = async (options = {}) => {
	...
	const outPath = (0, node_path.resolve)(cwd, out);
	const warnings = [];
	let config = require_config.getDefaultCnConfig();
	if (options.config) {
		const ext = await loadConfig(cwd, options.config);
		config = typeof ext === "function" ? ext(config) : require_compiler.mergeConfigs(config, ext);
	}
	const tokens = /* @__PURE__ */ new Set();
	let scannedFiles = 0;
	if (!options.full) {
		if (options.tokens) addWords(readText((0, node_path.resolve)(cwd, options.tokens), "tokens file", options.tokens), tokens);
		else {
			...
			for (const f of files) try {
				skippedLong += extractTokens((0, node_fs.readFileSync)(f, "utf8"), tokens);
				scannedFiles++;
			} catch {
				state.skippedUnreadable++;
			}
		...
		if (options.safelist) addWords(readText((0, node_path.resolve)(cwd, options.safelist), "safelist", options.safelist), tokens);
	}
	const fullConfig = config;
	let usedGroups = null;
	let totalGroups = null;
	if (!options.full) {
		const r = require_compiler.subsetConfig(config, tokens);
		config = r.config;
...
//   import tables from "./${(0, node_path.basename)(outPath)}"
//   import { createCn } from "cn/engine"
//   export const cn = createCn(tables)`;
	const source = require_compiler.compileToSource(config, {
		lang,
		banner
	});
	let changed;
	try {
		changed = (0, node_fs.readFileSync)(outPath, "utf8") !== source;
	} catch {
		changed = true;
	}
	if (changed) try {
		(0, node_fs.mkdirSync)((0, node_path.dirname)(outPath), { recursive: true });
		(0, node_fs.writeFileSync)(outPath, source);
	} catch (err) {
...
const createBuilder = (options = {}) => {
	...
	const changed = async (file) => {
		...
		const abs = (0, node_path.resolve)(cwd, file);
		if (abs === last.outPath || !matches(abs)) return last;
		let text;
		try {
			text = (0, node_fs.readFileSync)(abs, "utf8");
		} catch {
			return last;
		}
		...
		const reached = require_compiler.subsetConfig(last.fullConfig, fresh).config.classGroups;
...
exports.createContentMatcher = createContentMatcher;
