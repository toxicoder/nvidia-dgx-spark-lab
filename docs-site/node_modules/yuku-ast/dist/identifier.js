 ...
 */
export function isIdentifierChar(cp) {
    if (!Number.isInteger(cp) || cp < 0 || cp > MAX_CODE_POINT)
        return false;
 ...
 */
export function isReservedWord(word, inModule = false) {
 ...
 * friends.
 */
export function isStrictReservedWord(word, inModule = false) {
 ...
 * `arguments`.
 */
export function isStrictBindReservedWord(word, inModule = false) {
...
export function isValidIdentifier(name, reserved = true) {
    if (reserved && (isKeyword(name) || isStrictReservedWord(name, true)))
        return false;
