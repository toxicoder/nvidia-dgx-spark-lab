...
# =============================================================================
# 3-node LiteLLM rounded stack
# =============================================================================

@test "manage.sh start-stack-rounded aborts when user does not type 'yes'" {
  run bash -c "echo 'no' | bash \"$MANAGE_SH\" start-stack-rounded"
  [ "$status" -eq 0 ]
  [[ "$output" == *"Aborted"* ]]
}

@test "manage.sh start-glm53-flash aborts when user does not type 'yes'" {
  run bash -c "echo 'no' | bash \"$MANAGE_SH\" start-glm53-flash"
  [ "$status" -eq 0 ]
  [[ "$output" == *"Aborted"* ]]
}

@test "manage.sh start-stack-rounded refuses when Mode B job is active" {
  {
    echo '#!/usr/bin/env bash'
    echo "CALL_LOG=\"${TEST_TMP_DIR}/kubectl_calls.log\""
    cat << 'MOCKKUBECTL'
...
@test "manage.sh start-glm53-flash refuses when Mode A job is active" {
  {
    echo '#!/usr/bin/env bash'
    echo "CALL_LOG=\"${TEST_TMP_DIR}/kubectl_calls.log\""
    cat << 'MOCKKUBECTL'
...
@test "manage.sh stop-stack-rounded runs without error" {
  run bash "$MANAGE_SH" stop-stack-rounded
  [ "$status" -eq 0 ]
